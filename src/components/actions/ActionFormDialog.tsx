...
export function ActionFormDialog({ open, onOpenChange, user, associations, initial, onSaved }: Props) {
  ...
  const [budgetLines, setBudgetLines] = useState<BudgetLine[]>([
    { annee: String(currentYear), financeur: "", type: "", montant: 0 },
  ]);
  ...
  const totalBudget = budgetLines.reduce((s, l) => s + (Number(l.montant) || 0), 0);
  ...
  const handleSave = async () => {
    ...
    const cleanBudget = budgetLines
      .filter(b => b.financeur.trim() || b.montant)
      .map(b => ({ ...b, montant: Number(b.montant) || 0 }));
    const payload = {
      assoc_id: assocId,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Renseignez tous les champs de l'action</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {JOURS_OPTIONS.map(j => (
                <label key={j} className={`flex items-center gap-1 border rounded px-2 py-1 text-sm cursor-pointer ${jours.includes(j) ? "bg-primary text-primary-foreground border-primary" : ""}`}>
          ...
          <div className="col-span-2">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {QUARTIERS_OPTIONS.map(q => (
                <label key={q} className={`flex items-center gap-1 border rounded px-2 py-1 text-sm cursor-pointer ${quartiers.includes(q) ? "bg-primary text-primary-foreground border-primary" : ""}`}>
          ...
          <div className="col-span-2">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {TRANCHES_AGE_OPTIONS.map(t => (
                <label key={t} className={`flex items-center gap-1 border rounded px-2 py-1 text-sm cursor-pointer ${tranchesAge.includes(t) ? "bg-primary text-primary-foreground border-primary" : ""}`}>
          ...
          <div className="col-span-2">
            <div className="flex items-center justify-between">
              <Label>Budget — Financeurs</Label>
              <span className="text-sm font-bold text-primary">Total : {totalBudget.toLocaleString("fr-FR")} €</span>
            </div>
            <div className="space-y-2 mt-2">
              <div className="grid grid-cols-12 gap-2 text-xs text-muted-foreground px-1">
                <div className="col-span-2">Année</div>
                <div className="col-span-4">Financeur</div>
              ...
              {budgetLines.map((b, i) => (
                <div key={i} className="grid grid-cols-12 gap-2">
                  ...
                  <Input className="col-span-4" value={b.financeur} placeholder="ex : Orléans Métropole" onChange={(e) => {
                  ...
                  <Input className="col-span-2" type="number" value={b.montant} onChange={(e) => {
                    const n = [...budgetLines]; n[i] = { ...b, montant: Number(e.target.value) || 0 }; setBudgetLines(n);
                  }} />
              ...
              <Button type="button" size="sm" variant="outline" onClick={() => setBudgetLines([...budgetLines, { annee: String(currentYear), financeur: "", type: "", montant: 0 }])}>
